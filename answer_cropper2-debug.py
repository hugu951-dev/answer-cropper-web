...
def compute_divider_x_from_columns(columns: list[ColumnBand], page_width: int) -> int:
    if len(columns) < 2:
        return page_width // 2
    left = columns[0]
    right = columns[1]
...
def alpha_draw_mask(
    ...
    blended = image[selected].astype(np.float32)
    blended = blended * (1.0 - alpha) + color_arr * alpha
...
def alpha_draw_rect(
    ...
    cv2.rectangle(mask, (int(x1), int(y1)), (int(x2), int(y2)), 255, thickness=thickness)
    alpha_draw_mask(image, mask, color, alpha)
...
def render_debug_page_like_collect_entries(
    page_image: np.ndarray,
    anchors: list[QuestionAnchor],
    columns: list[ColumnBand],
    anchors_by_column: dict[int, list[QuestionAnchor]],
    foreground_mask: np.ndarray,
    split_x: int | None = None,
) -> np.ndarray:
    debug_image = page_image.copy()
    if split_x is None:
        split_x = compute_divider_x_from_columns(columns, page_image.shape[1])
    alpha_draw_line(
        debug_image,
        split_x,
        0,
        split_x,
        page_image.shape[0] - 1,
        color=(180, 0, 200),
        alpha=0.55,
        thickness=2,
    )
    ...
    for column_idx, column in enumerate(columns):
        ...
        if not column_anchors:
            continue
        row_color = (0, 0, 255) if column_idx == 0 else (0, 160, 0)
...
def write_debug_page(
    pdf_path: Path,
    output_path: Path,
    scale: float,
    page_number: int,
    force_midline: bool = False,
) -> None:
    ocr_engine = RapidOCR()
    ...
    split_x = page_image.shape[1] // 2 if force_midline else None
    if split_x is not None:
        provisional_left = [anchor for anchor in anchors if anchor.box.cx < split_x]
        provisional_right = [anchor for anchor in anchors if anchor.box.cx >= split_x]
        anchor_widths = [anchor.box.x2 - anchor.box.x1 for anchor in anchors]
        mean_anchor_width = (
            float(sum(anchor_widths) / len(anchor_widths))
            if anchor_widths
            else max(16.0, page_image.shape[1] * 0.03)
        )
        columns = [
            build_column_band_from_anchors(
                page_image.shape[1],
                provisional_left,
                mean_anchor_width,
                ColumnBand(0, max(0, split_x - 1)),
            ),
            build_column_band_from_anchors(
                page_image.shape[1],
                provisional_right,
                mean_anchor_width,
                ColumnBand(min(page_image.shape[1], split_x), page_image.shape[1]),
            ),
        ]
        anchors_by_column = {idx: [] for idx in range(len(columns))}
        for anchor in anchors:
            anchors_by_column[choose_column(anchor, columns)].append(anchor)
    debug_image = render_debug_page_like_collect_entries(
        page_image=page_image,
        anchors=anchors,
        columns=columns,
        anchors_by_column=anchors_by_column,
        foreground_mask=foreground_mask,
        split_x=split_x,
    )
    write_crop(debug_image, output_path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--debug-image",
        default=None,
        help="If set, render a debug overlay image for a single page to this path.",
    )
    parser.add_argument(
        "--debug-page",
        type=int,
        default=1,
        help="1-based page number to use with --debug-image.",
    )
    parser.add_argument(
        "--debug-force-midline",
        action="store_true",
        help="For debug rendering only, draw the column split line at the exact page center.",
    )
    return parser
...
def main() -> None:
    ...
    if args.debug_image:
        write_debug_page(
            ...
            scale=args.scale,
            page_number=args.debug_page,
            force_midline=args.debug_force_midline,
        )
        print(f"Input PDF: {pdf_path}")
        print(f"Debug page: {args.debug_page}")
        print(f"Debug image: {Path(args.debug_image).resolve()}")
        return
